...
module.exports = async function (m, conn = { user: {} }) {
  ...
  let me = '╭──────────═┅═──────────' + '\n│◪ 📞 Bot :' + PhoneNumber('+' + (conn.user && conn.user.jid).replace('@s.whatsapp.net', '')).getNumber('international')
  console.log(`
${chalk.blueBright('%s')} ${chalk.white('\n│◪ 🕔 Time : ' + '%s')} ${chalk.black(chalk.bgGreen('%s'))} ${chalk.magenta('\n│◪ 📥 Size : ' + '%s [%s %sB]')}${chalk.yellow('\n│◪ 👤 Form : ' + '%s%s')} ${chalk.green('\n│◪ 🗒️ TextSize : ' + '%s')}${chalk.blueBright('\n│◪ 📲 To : ')} ${chalk.green('%s')} ${chalk.redBright('\n│◪ 🗃️ Type : ' + '%s')}
├──────────═┅═──────────╮
╰◪                     ◪╯`.trim(),
    me + ' ~' + conn.user.name,
